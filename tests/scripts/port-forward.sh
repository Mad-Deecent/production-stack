...
kubectl patch service vllm-router-service -n "$NAMESPACE" -p '{"spec":{"type":"NodePort"}}'
ip=$(minikube ip)
port=$(kubectl get svc vllm-router-service -n "$NAMESPACE" -o=jsonpath='{.spec.ports[0].nodePort}')

sleep 5

bash "tests/helm/$1/test.sh" "$ip" "$port"
